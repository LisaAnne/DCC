...
class transfer_net(object):
  ...
  def __init__(self, model, model_weights, orig_attributes, all_attributes, vocab):
    ...
    self.net = caffe.Net(self.model, weights_folder + self.model_weights + '.caffemodel', caffe.TRAIN)
  ...
  def direct_transfer(self, words, classifiers, closeness_metric, log=False, predict_lm = 'predict-lm', predict_im = 'predict-im', num_transfer=1, orig_net_weights=''): 
   ...
   self.net.save('%s%s.%s.caffemodel' %(weights_folder, self.model_weights, save_tag))
   
   save_string = 'Saved to: %s%s.%s.caffemodel' %(weights_folder, self.model_weights, save_tag) 
   print save_string 
   if log: log_write.writelines('%s\n' %(save_string)) 
  ...
  def delta_transfer(self, words, classifiers, closeness_metric, log=False, predict_lm = 'predict', predict_im = 'predict-im', num_transfer=3, orig_net_weights=''): 
   ...
   orig_net = caffe.Net(self.model, weights_folder + orig_net_weights + '.caffemodel', caffe.TRAIN)
   ...
   self.net.save('%s%s.%s_delta_%d.caffemodel' %(weights_folder, self.model_weights, save_tag, num_transfer))
   
   save_string = 'Saved to: %s%s.%s_delta_%d.caffemodel' %(weights_folder, self.model_weights, save_tag, num_transfer) 
   print save_string 
   if log: log_write.writelines('%s\n' %(save_string)) 
   if log: log_write.close()
   if log: print 'Log file saved to %s.' %log_file 
